...
# Global variables for sequences and renaming operations
active_sequences = {}
renaming_operations = {}
...
async def end_sequence(client, message: Message):
    ...
    # Sort files by quality and episode number
    sorted_files = sorted(
        file_list,
        key=lambda f: (
            detect_quality(f.get("file_name", "")),  # Sort by quality
            extract_episode_number(f.get("file_name", ""))  # Sort by episode number
        )
    )
